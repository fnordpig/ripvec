...
/// Overlap between adjacent windows in bytes (25% of window size).
/// Ensures definitions spanning a window boundary are captured in at
/// least one window.
const WINDOW_OVERLAP: usize = WINDOW_SIZE / 4;
...
pub fn chunk_file(
    ...
    if parser.set_language(&config.language).is_err() {
        return sliding_windows(path, source);
    }
    ...
    while let Some(m) = matches.next() {
        ...
        if let Some(node) = def_node {
            ...
            if content.len() > MAX_CHUNK_BYTES {
                chunks.extend(sliding_windows_with_name(path, content, &name, start_line));
            } else {
...
///
/// Each window is `WINDOW_SIZE` bytes with `WINDOW_OVERLAP` bytes of
/// overlap. Window boundaries are adjusted to line breaks to avoid
/// splitting mid-line.
fn sliding_windows(path: &Path, source: &str) -> Vec<CodeChunk> {
    ...
    while offset < bytes.len() {
        let raw_end = (offset + WINDOW_SIZE).min(bytes.len());
...
/// `MAX_CHUNK_BYTES`. Windows carry the definition name for search context.
...
mod tests {
    ...
    fn fallback_large_file_produces_windows() {
        // Create a file larger than MAX_CHUNK_BYTES with no function declarations
        ...
        let config = crate::languages::config_for_extension("js").unwrap();
        let chunks = chunk_file(Path::new("big.js"), &source, &config);
        assert!(
            chunks.len() > 1,
    ...
    fn large_definition_is_windowed() {
        // A Rust function larger than MAX_CHUNK_BYTES
